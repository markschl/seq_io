...
reader_plot <- function(data, facets) {
  ggplot(data, aes(reader, gb_per_s, fill=reader)) +
    stat_summary(fun.y=mean, geom='bar', width=0.9, colour='#222222', size=0.2) + 
...
for (fmt in levels(reader_cmp$format)) {
    data <- subset(reader_cmp, format == fmt)
    png(file.path(outdir, sprintf('bench_%s.png', fmt)), width=1400, height=35*length(unique(data$group))+150, res=200)
    print(reader_plot(data, c('seqlen', 'config')))
    dev.off()
    
    sub <- subset(data, seqlen == 500 & !grepl('(records|seq)', config, perl=T))
    png(file.path(outdir, sprintf('bench_%s_simple.png', fmt)), width=1400, height=35*length(unique(sub$group))+150, res=200)
