...
//
// pub fn parallel_each<P, O, W, F, Out>(parser: P, n_threads: u32, queue_len: usize, work: W, mut func: F) -> Result<(), P::Err>
//     where P: Reader,
...
// {
//     let reader = ResultSetParser(parser, PhantomData);
//
...
//
// pub struct ParallelFastaRecords<'a, 'b, O>
//     where O: Send + 'b + 'a,
// {
//     inner: &'a mut ParallelRecordsets<ResultSet<fasta::RecordSet, Vec<O>>, io::Error, ()>,
//     iter: Option<fasta::RecordSetIter<'b>>,
//     item: Option<fasta::RefRecord<'b>>,
// }
//
// impl<'a, 'b, O> ParallelFastaRecords<'a, 'b, O>
//     where O: Send + 'a + 'b,
//  {
//
//      pub fn proceed(&'b mut self) -> Option<io::Result<()>> {
...
//          }
//
//          self.iter = match self.inner.next() {
//              Some(d) => {
//                  let (d, _) = try_opt!(d);
//                  Some((&d.0).into_iter())
//              },
...
//          };
//          None
//          //self.iter.as_mut().unwrap().next().map(|d| Ok(d))
//      }
//
//      pub fn get(&self) -> Option<io::Result<&fasta::RefRecord>> {
//          self.item.as_ref().map(|i| Ok(i))
//      }
//
//     pub fn next(&mut self) -> Option<io::Result<&fasta::RefRecord>> {
//         //self.proceed();
//         (*self).get()
//     }
// }
//
//
// pub fn parallel_fasta<R, O, W, F, Out>(reader: fasta::Reader<R>, n_threads: u32, queue_len: usize, work: W, mut func: F) -> Out
//     where O: Default + Send,
//           R: io::Read + Send,
//           W: Send + Sync,
//           W: Fn(fasta::RefRecord, &mut O),
//           F: FnMut(ParallelFastaRecords<O>) -> Out,
// {
//     let reader = ResultSetParser(reader, PhantomData);
//
//     read_parallel::<_, _, _, _, Out>(reader, n_threads, queue_len, |d| {
//         let mut iter = d.0.into_iter();
//         let mut out: &mut Vec<O> = &mut d.1;
//         for mut x in out.iter_mut().zip(&mut iter) {
//             work(x.1, &mut x.0);
//         }
//         for i in iter {
//             // could place_back() syntax when stable
//             out.push(O::default());
//             work(i, out.last_mut().unwrap())
//         }
//
//     }, |records| {
//         let rec = ParallelFastaRecords {
//             inner: records,
//             iter: None,
//             item: None,
//         };
...
//
//
// pub struct ParallelRecords<'a, 'b, P, O>
//     where P: Reader,
//           P::DataSet: 'a,
//           P::Err: 'a,
//           &'b P::DataSet: IntoIterator,
//           O: Send + 'a,
//           'a: 'b,
// {
//     inner: &'a mut ParallelRecordsets<ResultSet<P::DataSet, Vec<O>>, P::Err, ()>,
//     iter: Option<::std::iter::Zip<<&'b P::DataSet as IntoIterator>::IntoIter, ::std::slice::Iter<'b, O>>>,
// }
//
// impl<'a, 'b, P, O> ParallelRecords<'a, 'b, P, O>
//     where P: Reader,
//           P::DataSet: 'a,
//           P::Err: 'a,
//           &'b P::DataSet: IntoIterator,
...
//  {
//
//     pub fn next(&'b mut self) -> Option<Result<(<&'b P::DataSet as IntoIterator>::Item, &'b O), P::Err>> {
//         if let Some(ref mut iter) = self.iter {
//             if let Some(d) = iter.next() {
//                 return Some(Ok(d));
//             }
...
//
//
// pub fn parallel_each2<P, O, W, F, Out>(parser: P, n_threads: u32, queue_len: usize, work: W, mut func: F) -> Result<(), P::Err>
//     where P: Reader,
//           for<'a> &'a P::DataSet: IntoIterator,
//           O: Default + Send,
//           W: Send + Sync,
//           W: Fn(<&P::DataSet as IntoIterator>::Item, &mut O),
//           F: FnMut(ParallelRecords<P, O>),
// {
//     let reader = ResultSetParser(parser, PhantomData);
//
//     read_parallel(reader, n_threads, queue_len, |d| {
